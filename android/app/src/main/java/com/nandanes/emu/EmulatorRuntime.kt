...
class SaveStateManager(private val context: Context) {
    ...
    fun writeCurrentFrameThumbnail(bridge: NativeBridge, output: File): Boolean {
        val argb = bridge.getFrameArgb8888()
        if (argb.size <= 2) return false
        val width = argb[0]
        val height = argb[1]
        if (width <= 0 || height <= 0) return false
        val pixels = argb.copyOfRange(2, argb.size)
    ...
    fun start() {
        ...
        executor.execute {
            ...
            while (running && playbackToken == token) {
                val chunk = bridge.consumeAudioSamples(8192)
                if (chunk.isNotEmpty()) {
                    val pcm = if (sourceSampleRate != outputSampleRate) {
                        resampleStereo(chunk, sourceSampleRate, outputSampleRate)
                    } else {
                        chunk
                    }
